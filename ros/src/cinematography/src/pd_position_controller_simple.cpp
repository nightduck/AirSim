...
{
    vel_cmd_ = cinematography::VelCmd();
    // ROS params
    double update_control_every_n_sec;
    nh_private_.getParam("update_control_every_n_sec", update_control_every_n_sec);

    // ROS publishers
    airsim_vel_cmd_world_frame_pub_ = nh_private_.advertise<cinematography::VelCmd>("/vel_cmd_world_frame", 1);
...
bool PIDPositionController::local_position_goal_srv_cb(cinematography::SetLocalPosition::Request& request, cinematography::SetLocalPosition::Response& response)
{
...
bool PIDPositionController::local_position_goal_srv_override_cb(cinematography::SetLocalPosition::Request& request, cinematography::SetLocalPosition::Response& response)
{
    // this tells the update timer callback to not do active hovering 
    if(!got_goal_once_)
        got_goal_once_ = true;
...
void PIDPositionController::home_geopoint_cb(const cinematography::GPSYaw& gps_msg)
{
    if(has_home_geo_)
        return;
...
bool PIDPositionController::gps_goal_srv_cb(cinematography::SetGPSPosition::Request& request, cinematography::SetGPSPosition::Response& response)
{
    if(!has_home_geo_)
    {
...
// todo do relative altitude, or add an option for the same?
bool PIDPositionController::gps_goal_srv_override_cb(cinematography::SetGPSPosition::Request& request, cinematography::SetGPSPosition::Response& response)
{
    if(!has_home_geo_)
    {
        ROS_ERROR_STREAM("[PIDPositionController] I don't have home GPS coord. Can't go to GPS goal!");
